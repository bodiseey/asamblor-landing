...
export function generateMetadata({ params }: { params: { slug: string; state: string; city: string } }): Metadata {
  ...
  return {
    ...
    keywords: [
      ...
      `${s.name.toLowerCase()} ${city.name} ${state.name}`,
      `${s.name.toLowerCase()} near ${city.name}`,
      `${s.name.toLowerCase()} in ${city.name} ${state.abbr}`,
      `${audienceWord} hiring ${city.name}`,
      `${audienceWord} recruiting ${city.name}`,
      `${audienceWord} recruiting ${city.name} ${state.name}`,
      `find ${audienceWord} in ${city.name}`,
      `how to recruit ${audienceWord} in ${city.name}`,
      `${audienceWord} jobs ${city.name} ${state.abbr}`,
      `trucking company hiring ${city.name}`,
      `motor carrier acquisition pipeline ${city.name}`,
      `truck driver recruiting ${city.name} ${state.abbr}`,
      `owner operator opportunities ${city.name} ${state.abbr}`,
      `cdl driver opportunities ${city.name} ${state.abbr}`,
      `trucking jobs ${city.name} ${state.abbr}`,
      `${city.name} trucking acquisition`,
      "carriex",
      "asamblor",
    ],
  };
