...
export function generateMetadata({ params }: { params: { slug: string; state: string } }): Metadata {
  ...
  return {
    ...
    keywords: [
      ...
      `${s.name.toLowerCase()} ${state.abbr}`,
      `${s.name.toLowerCase()} prospects ${state.abbr}`,
      `${s.name.toLowerCase()} in ${state.name}`,
      `how to find ${s.name.toLowerCase()} in ${state.name}`,
      `${s.name.toLowerCase()} pipeline ${state.name}`,
      `motor carrier outreach ${state.name}`,
      `motor carriers ${state.name}`,
      `trucking companies in ${state.name}`,
      `${state.name} trucking sales leads`,
      `carrier acquisition ${state.abbr}`,
      `owner operator recruiting ${state.name}`,
      `cdl driver hiring ${state.name}`,
      `truck driver recruiting ${state.name}`,
      ...state.hubs.slice(0, 3).map((h) => `${s.name.toLowerCase()} ${h.split(" (")[0]}`),
      ...state.corridors.slice(0, 2).map((c) => `trucking ${c.split(" ")[0]} corridor ${state.abbr}`),
      "carriex",
      "asamblor",
      "trucking acquisition infrastructure",
    ],
  };
}
